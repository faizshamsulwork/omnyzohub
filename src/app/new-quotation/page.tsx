...
export default function NewQuotationWizard() {
  ...
  const nextStep = () => {
    if (step === 1 && !formData.client_name) return toast.error("Please select a customer.");
    setStep(prev => prev + 1);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!error) {
      toast.success(`Quotation ${formData.quote_no} created successfully.`, { id: loadingToast });
      router.push("/quotations");
      router.refresh();
    } else {
      toast.error(`Error saving quotation: ${error.message}`, { id: loadingToast });
      setLoading(false);
    }
  };

  return (
    <div className="min-h-screen p-8 md:p-12 selection:bg-blue-200 relative z-10 transition-colors duration-500 pb-32">
      <div className="max-w-5xl mx-auto">
        ...
        {isFetching ? (
          <div className="bg-white/90 dark:bg-[#111111]/90 backdrop-blur-xl p-8 md:p-20 rounded-[32px] shadow-2xl dark:shadow-gray-950/50 border border-gray-200 dark:border-gray-800 flex flex-col items-center justify-center min-h-[400px] animate-in fade-in duration-500">
            ...
            <p className="text-gray-500 font-bold text-sm animate-pulse tracking-widest uppercase">Connecting to Database...</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="bg-white/90 dark:bg-[#111111]/90 backdrop-blur-xl p-8 md:p-10 rounded-[32px] shadow-2xl dark:shadow-gray-950/50 border border-gray-200 dark:border-gray-800 transition-colors duration-500">
            ...
            <div className="mt-12 pt-6 border-t border-gray-100 dark:border-gray-800 flex justify-between">
              ...
              {step < 3 ? <button type="button" onClick={(e) => { e.preventDefault(); nextStep(); }} className="px-8 py-3 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-full shadow-lg shadow-blue-500/30 transition-all active:scale-95">Continue &rarr;</button>
              : <button type="submit" disabled={loading} className="px-10 py-3 text-sm font-bold text-white bg-green-600 hover:bg-green-500 rounded-full shadow-lg shadow-green-500/30 transition-all active:scale-95 disabled:opacity-50">{loading ? "Creating..." : "Finish & Create Quotation"}</button>}
            </div>
          </form>
        )}
      </div>
