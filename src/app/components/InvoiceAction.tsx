...
import { getErrorMessage, isPaidStatus } from "../lib/utils";
...
export default function InvoiceAction({
  ...
  const toggleStatus = async () => {
    ...
    const invoiceIsPaid = isPaidStatus(invoice?.status);
    const newStatus = invoiceIsPaid ? 'outstanding' : 'paid';
    const amountPaid = newStatus === 'paid' ? Number(invoice.amount) || 0 : 0;
  ...
  return (
    <div className="flex items-center justify-end gap-2 md:gap-3">
      ...
      >
        {isProcessing ? "..." : isPaidStatus(invoice?.status) ? "MARK UNPAID" : "MARK PAID"}
      </button>
