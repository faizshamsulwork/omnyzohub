...
export default function NewInvoiceWizard() {
  ...
  const [formData, setFormData] = useState({
    client_name: "", invoice_no: "Generating...", status: "outstanding",
    notes: "", terms: invoiceTemplates.standard
  });
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      if (lastInvoice && lastInvoice.length > 0 && lastInvoice[0].invoice_no) {
        ...
        setFormData(p => ({ ...p, invoice_no: `${prefix}${nextNum.toString().padStart(2, '0')}` }));
      } else {
  ...
  const handleClientSelect = (clientName: string, contactList = contacts) => {
    ...
    if (client) setClientInfo({ pic_name: client.pic_name || "", phone: client.phone || "", email: client.email || "", address: `${client.address || ""}, ${client.postcode || ""} ${client.city || ""}, ${client.state || ""}` });
  };
  ...
  const handleEnterKey = (e: React.KeyboardEvent<HTMLInputElement>) => { if (e.key === 'Enter') { e.preventDefault(); addItem(); } };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const mainDesc = items.filter(i => i.type === 'item').map(i => i.description).join('\n') || "Creative Services";

    const { error } = await supabase.from("invoices").insert([{
      invoice_no: formData.invoice_no, client_name: formData.client_name, description: mainDesc, amount: grandTotal, status: formData.status
    }]);
    ...
    <div className="min-h-screen p-8 md:p-12 selection:bg-blue-200 relative z-10 transition-colors duration-500">
      <div className="max-w-5xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="bg-white/90 dark:bg-[#111111]/90 backdrop-blur-xl p-8 md:p-10 rounded-[32px] shadow-2xl dark:shadow-gray-950/50 border border-gray-200 dark:border-gray-800 transition-colors duration-500">
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
              ...
              <div className="space-y-4">
                ...
                {items.map((item, index) => (
                  item.type === 'title' ? (
                    <div key={item.id} className="flex items-center bg-blue-50/50 dark:bg-blue-900/10 p-3 rounded-xl border border-blue-100 dark:border-blue-900/30"><div className="flex-1 w-full"><input type="text" placeholder="Section Title" required className="w-full bg-transparent border-none text-sm font-bold text-blue-700 dark:text-blue-400 focus:ring-0 p-2" value={item.description} onChange={(e) => handleItemChange(index, 'description', e.target.value)} onKeyDown={handleEnterKey}/></div>{items.length > 1 && <button type="button" onClick={() => removeItem(item.id)} className="text-red-400 hover:text-red-600 w-8 text-center">&times;</button>}</div>
                  ) : (
                    <div key={item.id} className="flex flex-col md:flex-row gap-4 items-start md:items-center bg-gray-50 dark:bg-[#0A0A0A] p-4 rounded-2xl border border-gray-200 dark:border-gray-800">
                      <div className="flex-1 w-full"><textarea placeholder="Item description" required rows={3} className="w-full bg-transparent border border-gray-200 dark:border-gray-700 rounded-lg text-sm text-gray-900 dark:text-white focus:outline-none p-3 resize-y" value={item.description} onChange={(e) => handleItemChange(index, 'description', e.target.value)}></textarea></div>
          ...
          {/* STEP 3: TERMA & NOTA (DENGAN TEMPLATE) */}
          {step === 3 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
